...
def upload_results(output, run, project, version):
    run_metadata = {"test_run": run, "environment": "dev", "branch": "main", "version": version, "build": "1.0.0", "project": project}
    ...
    test_results_columns = ["test", "status", "elapsedtime", "testsuite", "metadata", "starttime", "endtime"]
    ...
    # Write to database
    test_run_results_df.to_sql('testrun_results', engine, if_exists='append', index=True, dtype={'metadata': JSONB})

    testsuite_results_df.to_sql('testsuite_results', engine, if_exists='append', index=True, dtype={'metadata': JSONB})
    print("Done!")
    testresults_df.to_sql('test_results', engine, if_exists='append', index=True, dtype={'metadata': JSONB})
    testresults_json_df.to_sql('test_results_json', engine, if_exists='append', index=True, dtype={'json': JSONB})
...
def main():
    ...
    inputs=parse_args()
    print(inputs)
    upload_results(output=inputs.output, run=inputs.run, project=inputs.project, version=inputs.version)
