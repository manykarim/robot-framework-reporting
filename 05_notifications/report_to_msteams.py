...
with open("suite_overview.md", "w") as f:
    writer = MarkdownTableWriter(
        table_name="Test Status", headers=table_columns, value_matrix=suite_results
    )
    writer.stream = f
    writer.write_table()
    f.write("\n\n<br>\n\n")
...
with open("test_results.md", "w") as f:
    ...
    for suite in test_results:
        table_columns = [
            "Test Case <img width=100/>",
            "Status <img width=70/>",
            "Message",
            "Elapsed Time ⏱️ <img width=100/>",
        ]
        writer = MarkdownTableWriter(
            table_name=f"Failed and skipped tests in {suite}",
            headers=table_columns,
            value_matrix=test_results[suite],
        )
        writer.stream = f
        writer.write_table()
        f.write("\n\n<br>\n\n")
